...
PAGES = [
    ...
    ("session-detail", "/sessions/resp_tour_hybrid/", "Session detail (Django)"),
    ("profiles", "/profiles/", "LLM profiles (Django)"),
]

# Isolated XDG + config so captures do not leak ~/.config/swarm test profiles
# or the operator's 100+ live /v1/responses rows.
_CAPTURE_HOME = Path(tempfile.mkdtemp(prefix="os-journey-"))
_CAPTURE_CONFIG = _CAPTURE_HOME / "config" / "swarm" / "swarm_config.json"
_CAPTURE_RESPONSES = _CAPTURE_HOME / "data" / "swarm" / "responses"
...
SERVER_ENV = {
    ...
    "SWARM_TEST_MODE": "1",
    "XDG_CONFIG_HOME": str(_CAPTURE_HOME / "config"),
    "XDG_DATA_HOME": str(_CAPTURE_HOME / "data"),
    "XDG_CACHE_HOME": str(_CAPTURE_HOME / "cache"),
    "SWARM_CONFIG_PATH": str(_CAPTURE_CONFIG),
    "SWARM_RESPONSES_DIR": str(_CAPTURE_RESPONSES),
    # Uncomment to exercise token auth instead of open dev access:
    # "API_AUTH_TOKEN": "local-journey-token",
}


def prepare_capture_data() -> None:
    """Write a demo LLM profile and a few Session Explorer rows."""
    _CAPTURE_CONFIG.parent.mkdir(parents=True, exist_ok=True)
    _CAPTURE_RESPONSES.mkdir(parents=True, exist_ok=True)
    _CAPTURE_CONFIG.write_text(json.dumps({
        "llm": {
            "local-gateway": {
                "provider": "openai",
                "model": "auxiliary",
        ...
        "settings": {"default_llm_profile": "local-gateway"},
        "blueprints": {},
        "mcpServers": {},
    }, indent=2))
    demo = [
        {
            ...
            "model": "hybrid_team",
            "created": 3,
            "output": "Drafted a three-step rollout: inventory blueprints, launch one team, then inspect the session.",
            "progress": [
                {"role": "planner", "status": "completed", "model_used": "auxiliary", "result": "plan ready"},
                {"role": "writer", "status": "completed", "model_used": "auxiliary", "result": "draft ready"},
            ],
        },
        {
            "id": "resp_tour_chat",
        ...
        },
        {
            ...
            "model": "hybrid_team",
            "created": 1,
            "output": "Gathering specialist replies…",
            "progress": [
                {"role": "planner", "status": "completed"},
                {"role": "writer", "status": "in_progress"},
            ],
        },
    ]
    for row in demo:
        ...
        record = {
            ...
            "owner": "user:journey-admin",
        }
        path = _CAPTURE_RESPONSES / f"{rid}.json"
...
def capture(page, slug: str, path: str, name: str,
        ...
        try:
            ...
            page.evaluate(
                """([fromPath, toPath]) => {
                  ...
                  // Chromium full-page stitch clips the first few CSS pixels of
                  // a top-edge sticky strip; pad so "Redirected" stays intact.
    ...
    # Also expand inner scrollers (Session Explorer) and drop leftover focus rings.
    try:
        page.evaluate(
            """() => {
              ...
              const nodes = document.querySelectorAll(
                ...
                + ' .os-header, #os-capture-redirect-banner, nav.sticky, header.sticky-top, .sticky.top-0'
              );
              nodes.forEach((n) => {
              ...
              document.querySelectorAll('.se-list-scroll').forEach((n) => {
