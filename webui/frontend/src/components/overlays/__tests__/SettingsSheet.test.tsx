...
function renderSheet(isOpen = true, initialSection?: 'llm-profiles' | 'computer-control' | 'role') {
  ...
  const view = render(
    <QueryClientProvider client={client}>
      <ToastProvider>
        <SettingsSheet isOpen={isOpen} onClose={onClose} initialSection={initialSection} />
      </ToastProvider>
    </QueryClientProvider>,
  )
...
describe('SettingsSheet', () => {
  ...
  it('lists LLM models from the existing API when present', async () => {
    ...
    renderSheet(true, 'llm-profiles')
    expect(await screen.findByText('default')).toBeInTheDocument()
  })

  it('shows computer-control default and greyed TODO rows', () => {
    renderSheet(true, 'computer-control')
    expect(screen.getByRole('radio', { name: 'Browser (this machine)' })).toBeChecked()
    expect(screen.getByRole('radio', { name: 'Sandbox / Docker' })).toBeDisabled()
    expect(screen.getByRole('radio', { name: 'SaaS' })).toBeDisabled()
  })

  it('explains the support role without an LLM', () => {
    renderSheet(true, 'role')
    expect(screen.getByTestId('role-explanation')).toHaveTextContent(/Socratic helper/i)
  })
