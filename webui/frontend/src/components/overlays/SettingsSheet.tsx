...
export interface SettingsSheetProps {
  isOpen: boolean
  onClose: () => void
  /** Open a specific pane (LLM profiles, computer-control, role). */
  initialSection?: SettingsSection
  roleId?: string
}

/**
 * Right-docked DaisyUI settings sheet (REQ-48).
 *
 * Opens as `modal` + `modal-end` over the SPA (not a route that unmounts Chat).
 * Django `/settings/` stays the operator dump for power users.
 */
export default function SettingsSheet({
  isOpen,
  onClose,
  initialSection = 'retention',
  ...
  return (
    ...
    >
      <div className="flex min-h-[24rem] flex-1 flex-col overflow-hidden rounded-box border border-base-300 sm:flex-row">
        <nav aria-label="Settings sections" className="w-full shrink-0 border-b border-base-300 bg-base-200 sm:w-52 sm:border-b-0 sm:border-r">
          <ul className="menu menu-md w-full rounded-none p-2">
            ...
            <li>
              <button
                type="button"
                className={section === 'computer-control' ? 'menu-active' : undefined}
                aria-current={section === 'computer-control' ? 'page' : undefined}
                onClick={() => setSection('computer-control')}
              >
                Computer control
              </button>
            </li>
            <li>
              <button
                type="button"
                className={section === 'role' ? 'menu-active' : undefined}
                aria-current={section === 'role' ? 'page' : undefined}
        ...
        <div className="min-w-0 flex-1 overflow-y-auto bg-base-100 p-4 sm:p-5">
          ...
          {section === 'computer-control' && <ComputerControlPane />}
          {section === 'role' && <RoleDefinitionPane roleId={roleId} />}
        </div>
      </div>
