...
describe('AgentSidebar Grok rail', () => {
  ...
  it('REQ-171B: Add-agent CLI/API customs with rail true appear on the unpinned list', async () => {
    const seats = [
      blueprint('support', 'Support', 'Onboarding', 'support'),
      {
        ...blueprint('desk_cli', 'Desk CLI', 'CLI: grok'),
        kind: 'cli',
        command: 'grok',
        cli: 'grok',
        rail: true,
      ...
      {
        ...blueprint('researcher', 'Researcher', 'API assistant'),
        kind: 'api',
        rail: true,
      },
      { ...blueprint('scratch', 'Scratch', 'Catalog only'), rail: false },
    ]
    vi.stubGlobal('fetch', mockFetch(seats))
    renderSidebar('/chat')
  ...
  it('REQ-171B: completing Add-agent CLI create bumps the new seat to top of unpinned', async () => {
    const createdSeat = {
      ...blueprint('my_cli_tool', 'My CLI Tool', 'CLI: custom-tool'),
      kind: 'cli',
      command: 'custom-tool',
      cli: 'custom-tool',
      rail: true,
    ...
    const base = mockFetch(blueprints)
    vi.stubGlobal(
      'fetch',
      vi.fn().mockImplementation(async (input: RequestInfo, init?: RequestInit) => {
        ...
        if (url.includes('/v1/agent-settings') || url.includes('/v1/agents/')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ agent_id: 'my_cli_tool', folder: '' }),
          } as Response
        }
        if (
          method === 'GET' &&
          ...
          const data = includeCreated ? [...blueprints, createdSeat] : blueprints
          return {
            ok: true,
            status: 200,
    ...
    const addNew = screen.queryByTestId('empty-add-btn')
    if (addNew) fireEvent.click(addNew)
