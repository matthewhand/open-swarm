...
export const Accordion = ({
  ...
  const toggleItem = (key: string, disabled: boolean = false, e: React.SyntheticEvent<HTMLDetailsElement>) => {
    e.preventDefault();
    if (disabled) return;
        ...
        return (
          ...
          >
            <summary
              ...
              onClick={(e) => toggleItem(item.key, item.disabled, e)}
            >
...
export const AccordionItem = ({
  ...
  const handleToggle = (e: React.SyntheticEvent<HTMLDetailsElement>) => {
    if (disabled) {
      e.currentTarget.open = isOpen; // revert to internal state
    } else {
