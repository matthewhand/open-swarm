...
> use an isolated config + a few demo `/v1/responses` rows so host
> `~/.config/swarm` fixtures and live session history do not leak into docs.
> Empty states (no registered teams, empty My Blueprints) are called out in
> captions. Terminal transcripts below were captured 2026-06-10 on `main`.
