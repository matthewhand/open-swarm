...
def _bp(tmp_path: Path, **params) -> MoAOrchestratorBlueprint:
...
async def test_moa_orchestrator_blueprint_multi_task(tmp_path: Path):
    ...
    assert (tmp_path / "decision.md").is_file()
    assert (tmp_path / "test_notes.md").is_file()
    assert (tmp_path / "docs" / "ADR.md").is_file()
    assert (tmp_path / "moa_determination.md").is_file()


@pytest.mark.asyncio
async def test_moa_orchestrator_at_path_custom_outputs(tmp_path: Path):
    ...
    final = chunks[-1]
    assert final.get("final") is True
    assert (tmp_path / "artifacts" / "decision.md").is_file()
    assert (tmp_path / "qa" / "test_notes.md").is_file()
    assert (tmp_path / "docs" / "custom_ADR.md").is_file()
    ...
    assert not (tmp_path / "test_notes.md").exists()


@pytest.mark.asyncio
async def test_moa_orchestrator_tasks_list_of_dicts(tmp_path: Path):
    ...
    assert (tmp_path / "out" / "impl.md").is_file()
    assert (tmp_path / "research_notes.md").is_file()  # researcher default


@pytest.mark.asyncio
async def test_moa_orchestrator_team_tasks_alias(tmp_path: Path):
    """CLI-style team_tasks alias works when tasks is unset."""
    bp = _bp(tmp_path, team_tasks="implementer:go@out.md")
...
@pytest.mark.asyncio
async def test_moa_orchestrator_default_implementer_when_no_tasks(tmp_path: Path):
    bp = _bp(tmp_path)
    bp._params.pop("tasks", None)
...
@pytest.mark.asyncio
async def test_moa_orchestrator_participants_csv_string(tmp_path: Path):
    bp = _bp(tmp_path, participants="analyst, critic", tasks="implementer:x")
    chunks = []
