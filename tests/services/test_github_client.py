...
# session. Instead, patch the model objects on the imported module per-test.
...
def reset_mocks(monkeypatch):
    ...
    monkeypatch.setattr(github_client_module, "Blueprint", mock_models.Blueprint, raising=False)
    monkeypatch.setattr(github_client_module, "MCPConfig", mock_models.MCPConfig, raising=False)
    yield
