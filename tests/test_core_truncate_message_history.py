...
def test_truncate_by_token_count(mode_env_var, patch_get_token_count):
    ...
    # Corrected Assertions: Simple logic results in length 3 here.
    expected_len = 3
    expected_content = ['Sys', 'Long Response', 'Short User 2']

    assert len(truncated) == expected_len, f"Mode {mode_env_var} failed length check: Expected {expected_len}, got {len(truncated)}. Result: {truncated}"
    assert truncated[0]["content"] == expected_content[0], f"Mode {mode_env_var} failed content check (index 0)."
    assert truncated[1]["content"] == expected_content[1], f"Mode {mode_env_var} failed content check (index 1)."
    assert truncated[2]["content"] == expected_content[2], f"Mode {mode_env_var} failed content check (index 2)."
