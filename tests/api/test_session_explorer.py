...
def test_preview_output_text_strips_rest_plan_markers():
    raw = (
        "REST plan:\n[rest-plan] hi\n\ngrok persona:\nGROK:hi\n\n"
        "--- Plan ---\n[rest-plan] hi\n\nConsensus:\nA:hi\n"
    )
    preview = responses_store.preview_output_text(raw)
    assert "[rest-plan]" not in preview
    assert "REST plan" not in preview
    assert "Consensus:" not in preview
    assert "GROK:hi" in preview
    assert "A:hi" in preview


def test_list_summaries_uses_sanitized_preview(store):
    _save("resp_p", output="REST plan:\n[rest-plan] hi\n\nConsensus:\nParis")
    rows = responses_store.list_summaries()
    assert rows[0]["output_preview"]
    assert "[rest-plan]" not in rows[0]["output_preview"]
    assert "Paris" in rows[0]["output_preview"]
