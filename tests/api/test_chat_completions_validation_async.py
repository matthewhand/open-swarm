"""Async request-validation and error-path tests for /v1/chat/completions.

Ported from archive/local-main-2025-04
(tests/api/test_chat_completions_validation_async.py) and adapted to the
current API layer (serializer error messages and view exception text verified
against src/swarm/serializers.py and src/swarm/views/chat_views.py on main).
"""
import json
...
def mock_get_blueprint_fixture():
    # The view awaits get_blueprint_instance, so patch with AsyncMock.
    with patch('swarm.views.chat_views.get_blueprint_instance', new_callable=AsyncMock) as mock_get_bp:
        ...
        mock_blueprint_instance.run = _mock_run_async_gen
...
class TestChatCompletionsValidationAsync:
    ...
    async def test_missing_required_field_returns_400(self, authenticated_async_client, field):
        ...
        assert field in response_data  # The specific field error is reported
    ...
    async def test_invalid_field_type_or_content_returns_400(self, authenticated_async_client, invalid_data, expected_error_part):
        ...
        # Check that the core part of the expected error message is present
        # anywhere in the string representation of the response JSON.
        ...
        assert error_found, (
            f"Expected error containing '{core_expected_error}' "
            f"(from '{expected_error_part}') not found in response: {response_data}"
        )
    ...
    async def test_nonexistent_model_not_found(self, authenticated_async_client, mocker):
        ...
        # get_blueprint_instance returning None must yield a 404.
    ...
    async def test_blueprint_init_error_returns_500(self, authenticated_async_client, mocker):
        ...
        # get_blueprint_instance raising simulates blueprint init failure.
    ...
    async def test_blueprint_run_exception_non_streaming_returns_500(self, authenticated_async_client, mocker):
        ...
        # Blueprint whose run() raises mid-generation.
        mock_blueprint_instance = MagicMock()
        ...
        mock_blueprint_instance.run = failing_run
