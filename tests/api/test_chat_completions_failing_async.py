"""Async streaming (SSE) tests for /v1/chat/completions.

Ported from archive/local-main-2025-04
(tests/api/test_chat_completions_failing_async.py — historical name kept for
salvage traceability; the tests pass on current main).

``ChatCompletionsView._handle_streaming`` is patched directly: it is an
``async def`` method, so ``mocker.patch.object`` auto-creates an ``AsyncMock``
whose return value (a pre-built ``StreamingHttpResponse``) is what the view's
``await self._handle_streaming(...)`` resolves to.
"""
import asyncio
import json
import time
...
async def mock_stream_generator(chunks: list):
    for chunk_str in chunks:
        yield chunk_str.encode('utf-8')  # StreamingHttpResponse expects bytes
        await asyncio.sleep(0.01)  # Simulate delay
    yield b"data: [DONE]\n\n"


@pytest.mark.django_db(transaction=True)
class TestChatCompletionsStreamingAsync:

    @pytest.fixture(autouse=True)
    def setup_general_mocks(self, mocker, test_user):  # test_user from tests/api/conftest.py
        mocker.patch('swarm.views.chat_views.validate_model_access', return_value=True)
        # Session auth is the primary path for these tests (authenticated_async_client).
        ...
        # get_blueprint_instance must return a non-None instance before _handle_streaming runs.
        mocker.patch(
    ...
    async def test_chatbot_streaming_success(self, authenticated_async_client, mocker):
        ...
        assert '"delta": {"role": "assistant", "content": "Chatbot"}' in content_str
    ...
    async def test_blueprint_run_exception_streaming_returns_error_sse(self, authenticated_async_client, mocker):
        ...
        # Simulate _handle_streaming catching an error and emitting an SSE error chunk.
        mocker.patch.object(ChatCompletionsView, '_handle_streaming', return_value=mock_streaming_response)
        ...
        # Streaming responses return 200 even when the stream itself carries an error.
        assert response.status_code == status.HTTP_200_OK
        assert response.get('content-type') == 'text/event-stream'
        ...
        assert 'data: [DONE]' in content_str  # DONE is still sent after an error
