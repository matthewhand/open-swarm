...
### Added
- **Remote harnesses (REQ-11):** persist `remotes.hermes|omb|rakazo` (base URL + auth), honest health/version, and operate via each product's real API. CLI `swarm-cli remotes`, REST `/v1/remotes/`, Settings group, blueprint `remote_harness` (openai-agents as_tool — not a Grok/OMB/Rakazo seat clone). Rakazo RPC remains Better-Auth-gated. Docs: [docs/REMOTE_HARNESSES.md](docs/REMOTE_HARNESSES.md).
