...
### Added
- **REQ-111 Wave 0 swarm-cli TUI scaffold:** ADR-012 records a Herdr-like left rail + chat pane over the same HTTP REST the WebUI uses (Textual recommended for Wave 1; not a Herdr SSH TUI). `swarm-cli tui --once` lists rail seats from `/v1/blueprints/`, `/v1/cli-agents/`, and `/v1/remotes/` and shows a placeholder chat pane. API down is an honest error; no fake agents. Default base `http://127.0.0.1:8000` (no `:8001`). Children: #874–#883. Does not close the #481 programme. (#481)
