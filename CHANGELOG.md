...
### Changed
- **REQ-48 chat stays mounted:** Settings, Teams, Blueprints, Hidden Bots, plugins, hostname, LLM profiles, role pane, and computer-control open as DaisyUI `modal` / `modal-end` sheets over Chat. Composer `+` and Search/⌘K open those overlays instead of navigating `/` away from the thread. Django `/settings/` remains an operator-dump link. No new React routes that unmount ChatPage.
