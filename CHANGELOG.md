...
### Changed
- **REQ-12 direction honesty:** [docs/VISION.md](docs/VISION.md) states the harness-of-harnesses thesis (Hermes / OpenMausBot / Rakazo via openai-agents handoff / `as_tool`, not extra concurrent seats) plus a short live-vs-intended table. **Teams collision:** live `/teams/` is an LLM-profile alias registry; intended Team wires API/CLI/remote agents to talk (handoff / `as_tool`) — Teams Admin does **not** do inter-agent talk. Grok-Bot-like UI is **not live**; Support/gate/skeptic are **in flight**; remotes (REQ-11) are **not landed**. README, GLOSSARY, ROADMAP pointers aligned. This is not the generic audit in PR #297.
