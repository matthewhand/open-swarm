...
### Added
- **REQ-63 Rakazo remote kind:** After **+ Add remote** (API base URL, optional UI URL, `api-key-env` and/or `session-cookie-env` names), Settings can health-check, list bots, and send through Rakazo. Unauthenticated `GET /health` may succeed while `/rpc/bots/list` and `/rpc/threads/send` report auth-needed (`gap=rakazo_rpc_requires_better_auth_session`) without crashing. Persist env-var names / `${NAME}` only — no pasted tokens or cookies. Stub HTTP tests; no live LAN. Fixes #389.
