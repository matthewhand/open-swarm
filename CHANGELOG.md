...
### Added
- **REQ-61 Hermes remote kind complete:** Remotes catalog is opt-in (empty until + Add remote). After add (base URL + `api_key_env` name only) Settings can health / list / send via the documented Hermes API (`GET /health`, `GET /v1/models` + `/api/sessions` + `/api/jobs`, `POST /v1/runs`). Missing remote is empty, not a default card. DOWN is a 200 report. Tests stub HTTP; no live LAN, no tokens. Reuses `swarm.core.remotes`.
