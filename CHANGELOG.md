...
### Added
- **REQ-62 OpenMousBot remote kind:** Settings Remotes start empty (+ Add remote). After add (base URL + optional api-key-env), OpenMousBot can health, list bots, and send to a bot id. Kind id stays `omb`; UI label is OpenMousBot, never OMB. HTTP stubs only — no live LAN, no tokens, no OpenMousBot source clone. Compatible with REQ-59 opt-in remotes.
