...
### Changed
- **REQ-34 Avatar theme sets:** Settings picks Default (today’s `os-agent-dot` marks) or Blobs. Choice persists in `localStorage.swarm_avatar_theme` (hostname-style) and applies to left-rail rows, favourite tiles, and the chat header when Blobs is on. Blobs are deterministic SVG shapes + two black capsule eyes; idle is a hashed rest pose, active (selected / streaming) is a slow wander. Theme switch does not rewrite blueprints.
