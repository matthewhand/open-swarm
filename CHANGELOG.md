...
### Added
- **REQ-159 three kind bases:** ADR-003 (`ApiKindBase` / `CliKindBase` / `RemoteKindBase` in `swarm.core.kind_bases`). Support + shared author brief prefer those templates over raw `BlueprintBase`. README openai-agents section cross-links. Creator validator accepts a kind base. Wizard/library emit paths stay `BlueprintBase` until follow-up. Fixes #570.
