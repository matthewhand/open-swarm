...
- **Team roster composition (REQ-20):** Chat-header **Compose team** `+` opens a two-pane overlay (grey dashed “drop agents here” + available API/CLI/remote list). Native HTML5 drag-drop and context-menu Add/Remove. Members show name, kind badge (`API`/`CLI`/`remote`), and role. Per-team `handoff` / `as_tool` toggles default on. Persist `team_rosters.json` via `/v1/team-rosters/` — **not** `teams.json` LLM aliases. Django `/teams/` admin/launcher stay aliases. Remotes/CLIs may be placeholders; they are not Blueprint classes. Gate unwired = all tools approved. Grok chrome stays left-rail + chat (no restored Home/Chat top nav). See [docs/TEAM_ROSTERS.md](docs/TEAM_ROSTERS.md).
