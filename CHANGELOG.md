...
- **REQ-105 Select / New session (Django):** Rail right-click on any API or CLI agent offers **Select session…** (shared search-palette chrome: filter, keyboard, title + relative time + snippet) and **New session**. Sessions are first-class Django rows scoped to the agent; messages and compact summaries belong to that session. Default stays one active session until the user opts in. Scale-out-created sessions appear in the same picker. CLI provider browse/import stays in #468. Teams/remotes omit the items. Fixes #469.
