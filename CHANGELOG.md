...
### Added
- **REQ-53 hostname rail icon:** left-rail footer hostname shows a muted connection icon immediately to the left of the name (inherits hostname colour — not success green). WebSocket drop / handshake failure swaps it for a red disconnected glyph (`text-error`); reconnect restores the bland icon. No icon when the hostname row is hidden.
