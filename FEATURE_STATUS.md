...
## 5. Web UI — React SPA (`webui/frontend`) — 🔲 1 · 🟡 3 · 🗑 3
...
| Browser control pane (REQ-45) | 🟡 | Header **Browser control** (`Globe`) opens a DaisyUI pane. **Browser (this machine)** is the selected default (Playwright launch/attach). **Sandbox / Docker** and **SaaS** are greyed TODO (clickable WIP, not wired). Desktop/OS stays #341. Driver: `swarm.core.browser_control` + stub tests; missing Chrome → structured error. Catalog `GET /v1/browser-control/`. No live preview. |
| Runtime banner (REQ-45) | 🟡 | Dismissible banner from `GET /v1/runtime/` (`SWARM_RUNTIME_MODE`). `bare-metal` / `sandbox-home` warning; `sandbox-isolated` green/info; missing/unknown never fakes green. Dismiss persists `localStorage.swarm_runtime_banner_dismissed` (re-shows if mode changes). Compose wires the env. Copy uses `$HOME` / `SWARM_SANDBOX_ROOT` only. |
