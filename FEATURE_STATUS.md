...
| Dropdown change in transcript (REQ-46) | ✅ | Any chat `<select>` (team target, CLI, model/profile, remote vs CLI) appends one centred `os-chat-status` line (`{kind}: {from} → {to}`). Persisted as `role=status` via `POST /chat/thread/` + WS `type=status` (no LLM). Hydrate on reload. Not a DaisyUI bubble. |
