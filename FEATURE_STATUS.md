...
| `/v1/team-rosters/` composition API | ✅ | `views/team_rosters_api.py` + `core/team_rosters.py` over **`team_rosters.json`** (`members[{id,kind,role,source}]` + `wires.handoff`/`as_tool`, both default on). Separate from `teams.json` aliases and from `/v1/agent-team/` config. `/v1/team-agents/` palette (API=blueprints, CLI=catalog or placeholders, remote=placeholders — not Blueprint classes). Gate unwired = all tools approved (REQ-314 not in tree). Tests: `tests/views/test_team_rosters_api.py` |
...
| Teams composition overlay (REQ-20) | ✅ | SPA `TeamComposer`: first-launch **two panes** (grey dashed “drop agents here” + available API/CLI/remote list). Native HTML5 drag-drop; context menu Add/Remove. Member chip = name + kind badge + role. Per-team handoff/as_tool toggles (default on). Persists `/v1/team-rosters/` → user-config `team_rosters.json`. Django `/teams/` unchanged (aliases). Entry is a **Compose team** control (not a SPA Teams tab / not a restored Home·Chat top nav). |
