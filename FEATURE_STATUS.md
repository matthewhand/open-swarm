...
| Scale-out rail + session picker (REQ-66) | ✅ | One rail row per agent. Session count > 1 → stacked avatars (max 3 + remainder), every face animated with `startedAt` stagger. Click opens search-palette overlay of running + finished sessions; row click sets `?session=` on still-mounted Chat. Empty copy “no sessions yet”. Tests: `scaleOutSessions.test.ts`, `StackedAvatars.test.tsx`, `SessionPicker.test.tsx`, AgentSidebar / ChatPage. |
