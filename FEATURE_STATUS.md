...
| `/v1/preferences/` (REQ-144) | ✅ | First-party `UserPreference` JSON bag keyed by User / token / guest session. Favourites + Hidden Bots. SPA import-once from localStorage when empty. Tests: `tests/views/test_preferences_api.py`. No Neon / no secrets. |
...
| User preferences API (REQ-144) | ✅ | First-party `UserPreference` (FK/OneToOne to User + `principal`, JSON `values` bag). `GET/PATCH /v1/preferences/` for ordered favourites + Hidden Bots. Logged-in → `user:<name>`; Bearer → `token:<hash>`; guest → Django session. No django-dynamic-preferences dep; no secrets; no Neon. Tests: `tests/views/test_preferences_api.py`, `tests/core/test_user_preferences.py`, `userPrefs.test.ts`. |
