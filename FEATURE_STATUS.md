...
| Teams in AGENTS sidepane (REQ-23) | ✅ | Sidepane mixes **team roster** rows (Users icon + Team badge) with agent rows from `GET /v1/team-rosters/` / `team_rosters.json` (never `/v1/teams` LLM aliases). Clicking a team opens `/chat?team=`; unlabeled dropdown lists **All members**, each member (name + kind/role), then **Manage Teams** (overlay, not eject). Send frames `{team, target: "all"\|memberId}`; runtime fan-out is stubbed. Empty roster keeps the row and disables All members. |
