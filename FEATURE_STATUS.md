...
| Kind bases API/CLI/remote (REQ-159) | ✅ | ADR-003 + stubs `swarm.core.kind_bases`. Support / `BLUEPRINT_AGENT_BRIEF` prefer kind templates. Creator validator accepts them. Cross-link from README openai-agents section. Wizard/library still emit `BlueprintBase` (follow-up). Fixes #570. |
