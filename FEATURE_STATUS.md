...
| Select / New session (REQ-105) | ✅ | Django `ChatConversation` is agent-scoped (title/snippet/labels/`cli_session_id`). `GET/POST /v1/agents/<id>/sessions/` lists and mints empty sessions (`{"new": true}`). Rail **Select session…** / **New session** on API and CLI rows; shared `SessionPicker` chrome (relative time + New). Chat `?session=` replaces transcript + compact tree; status “Switched to session …”. Teams/remotes omit. CLI provider list stays #468. Tests: `test_agent_sessions.py`, `test_agent_settings_api.py`, `test_req105_session_chrome.py`, `agentSessions.test.ts`, `SessionPicker.test.tsx`, `AgentSidebar.test.tsx`. |
