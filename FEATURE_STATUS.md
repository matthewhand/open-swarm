...
| Feature | Status | Evidence |
|---|---|---|
| Index/dashboard | ✅ | `web_views.index` serves SPA `dist/index.html` when present, else Django `index.html`; `tests/views/test_web_views.py::TestIndexView`. Docker image bakes `dist/` via multi-stage Node build (gitignored locally — `npm run build` or `scripts/build_frontend.sh`) |
...
| ChatMessage tenancy + prod security headers | ✅ | `message_views.py` scopes list to `conversation__student=request.user` when `ENABLE_API_AUTH`; token-only → empty qs. Prod-only (`DEBUG=False`): secure cookies (`SWARM_SECURE_COOKIES`); CSP via `ContentSecurityPolicyMiddleware` (`SWARM_CSP=false` to opt out; `script-src 'self'`, residual `style-src 'unsafe-inline'` — AUTH.md §7). Always-on (Django defaults + middleware, debug and prod): `SECURE_CONTENT_TYPE_NOSNIFF`, `X_FRAME_OPTIONS=DENY` (prod block reasserts / `DJANGO_X_FRAME_OPTIONS`). Operator UI assets self-hosted under `static/contrib/`. Browser honesty: `swarm.core.browser_tools` + TROUBLESHOOTING §7 (no fake playwright success). |
