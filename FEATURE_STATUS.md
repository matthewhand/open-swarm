...
| Dashboard (`/`) + Chat (`/chat`) | 🟡 | `App.tsx` routes only these two; desktop nav Home·Chat + Django hrefs (Blueprints·Teams·Sessions·Settings); **mobile five-tab dock** Home·Chat + Django hrefs (Blueprints·Teams·Sessions) — Settings is desktop top-nav/gear only (matches `mobile/*.png`). ChatPage WS via ASGI; needs Django **session cookie** (bearer does **not** auth websockets). Anonymous sockets close **4401**. Safe GFM markdown (`marked` + `htmlSafe`) + WS auto-reconnect (skips 4401). |
