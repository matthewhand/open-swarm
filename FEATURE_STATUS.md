...
| Safety tool badges + approval (REQ-55) | ✅ | API-agent chat popups: DaisyUI badge blue+animated running, green allowed/done, red denied/error. Role badge / UI copy say **Safety** (internal `gate`/`tool_gate`). Concerned calls pause with Allow once / Always allow / Deny; always-allow is per agent + tool name. Unwired or unconcerned = no prompt. CLI/remote do not call swarm approval. Tests: `test_safety.py`, `ToolCallPopup.test.tsx`, `ChatPage.test.tsx`. |
