...
| Settings default LLM + per-task override (REQ-43) | ✅ | SPA Settings → LLM profiles lists configured CLI/API/remote ids and a Default picker persisted as `settings.default_llm_profile` (`GET/PATCH /v1/llm-profiles/`). Auto-pick + optional alias prefill. Override-per-task routes summary → auxiliary and design → delegation; missing slug warns and uses Default. Chat `respond_with_default_model` honours that default when env is unset. #356 hook: `resolve_summary_model()`. Tests: `tests/core/test_llm_task_routing.py`, `tests/views/test_llm_profiles_api.py`, `SettingsSheet.test.tsx`. |
