...
def validate_message_sequence(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    valid_tool_call_ids = {tc["id"] for msg in messages if msg.get("role") == "assistant" for tc in (msg.get("tool_calls") or []) if isinstance(tc, dict) and "id" in tc}
...
def repair_message_payload(messages: List[Dict[str, Any]], debug: bool = False) -> List[Dict[str, Any]]:
    ...
    valid_tool_call_ids = {tc["id"] for msg in messages if msg.get("role") == "assistant" and isinstance(msg.get("tool_calls"), list) for tc in msg.get("tool_calls", []) if isinstance(tc, dict) and "id" in tc}
