...
def adapter_id_for_blueprint(blueprint_id: str | None, params: dict[str, Any] | None = None) -> str:
    """Map a rail / blueprint id to the empirical adapter table key."""
    from swarm.core.cli_catalog import cli_from_rail_id

    params = params or {}
    raw_cli = params.get("cli")
    if isinstance(raw_cli, str) and raw_cli.strip():
        return f"cli:{raw_cli.strip()}"
    text = (blueprint_id or "").strip()
    if not text:
        return "openai_compat"
    cli = cli_from_rail_id(text)
    if cli:
        return f"cli:{cli}"
    lowered = text.lower()
    ...
    if lowered.startswith("remote:"):
        return lowered
    for remote in ("hermes", "omb", "rakazo", "herdr", "swarm"):
...
def apply_speaker_identity(
    ...
    *,
    ...
    """Label real turns for ``adapter_id``. Chrome is already out of this list.
    ...
    * **delimiter** — wrap body; do not rely on a structured ``name`` field
    ...
    for item in turns_for_model(messages):
        row = dict(item)
