...
class RemoteSpec:
    ...
    def public_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "configured": self.source in ("config", "env"),
...
def is_configured(remote_id: str, config: dict[str, Any] | None = None) -> bool:
    """True when the user (or env) has added this kind. Defaults do not count."""
    rid = _require_kind_id(remote_id)
    cfg = config if isinstance(config, dict) else load_raw_config()[0]
    return rid in configured_remote_ids(cfg)
...
def load_configured_remotes(config: dict[str, Any] | None = None) -> dict[str, RemoteSpec]:
    """Configured remotes as an id → spec map. Empty dict until the user adds one."""
...
def add_remote(
    ...
    *,
    ...
    """Settings / CLI add: persist kind + URL + env-var names (not tokens)."""
