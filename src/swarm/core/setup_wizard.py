...
def run_setup_wizard(config_path: str, blueprints_metadata: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    while True:
        ...
        provider["model"] = input("Enter the model name (e.g., 'gpt-5.5'): ").strip()
        provider["base_url"] = input("Enter the base URL for the API (e.g., '${OPENAI_BASE_URL}'): ").strip()
        provider["api_key"] = input("Enter the environment variable for the API key (e.g., 'OPENAI_API_KEY' preferred) [Leave empty if not required]: ").strip()
