...
class DjangoChatConsumer(AsyncWebsocketConsumer):
    ...
    async def respond_with_default_model(self, contents_div_id):
        """Use the centralized OpenAI-compatible bootstrap for legacy replies.

        ``AsyncOpenAI`` remains a lazy module-level sentinel so tests and callers
        can patch it without importing the SDK during module initialization.
        """
        import swarm.consumers as _self_mod
        from swarm.utils.env_utils import get_openai_bootstrap

        client_class = _self_mod.AsyncOpenAI
        if client_class is None:
            from openai import AsyncOpenAI as client_class

            _self_mod.AsyncOpenAI = client_class

        bootstrap = get_openai_bootstrap()
        client_kwargs = {}
        if bootstrap:
            if bootstrap.get("api_key"):
                client_kwargs["api_key"] = bootstrap["api_key"]
            if bootstrap.get("base_url"):
                client_kwargs["base_url"] = bootstrap["base_url"]
