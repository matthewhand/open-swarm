...
class GitHubConfig:
    ...
    timeout: int = 30
    max_retries: int = 3
    user_agent: str = "Swarm-GitHub-Client"
...
class GitHubClient:
    ...
    def __init__(self, config: Optional[GitHubConfig] = None, **kwargs: Any):
        if config:
            ...
            self.config = GitHubConfig(**kwargs)
        else:
        ...
        *,
        ...
        try:
            ...
            with httpx.Client(
                ...
                resp = client.get(f"{self.base_url}/search/repositories", params=params)
    ...
    def fetch_manifests(
        ...
        try:
            ...
            with httpx.Client(
                ...
                top = client.get(f"{self.base_url}/repos/{owner}/{name}/contents/open-swarm.json")
                ...
                for base in ("swarm/blueprints", "swarm/mcp"):
                    resp = client.get(f"{self.base_url}/repos/{owner}/{name}/contents/{base}")
                    if resp.status_code != 200:
                        continue
                    ...
                    for ent in entries:
                        ...
                        if not path:
                            continue
                        man = client.get(f"{self.base_url}/repos/{owner}/{name}/contents/{path}/manifest.json")
                                ...
                                if content:
                                    try:
                                        ...
                                        if isinstance(parsed, dict):
                                            ...
                                            self.enrich_item_with_metrics(client, owner, name, path, parsed)
    ...
    def enrich_item_with_metrics(
        ...
        try:
            listing = client.get(f"{self.base_url}/repos/{owner}/{repo}/contents/{item_dir}")
            ...
            for f in entries:
                ...
                if size and size < 200_000:
                    ...
                    fc = client.get(f"{self.base_url}/repos/{owner}/{repo}/contents/{f.get('path')}")
    ...
    def sync_marketplace_items(self) -> tuple[int, int]:
        ...
        repos = self.search_repositories(topics, orgs)
        ...
        for repo in repos:
            ...
            manifests = self.fetch_manifests(repo)
            ...
            for manifest in blueprint_manifests:
                try:
                    ...
                    blueprint, created = Blueprint.objects.update_or_create(
                        name=name,
                        defaults={
            ...
            for manifest in mcp_manifests:
                try:
                    ...
                    mcp_config, created = MCPConfig.objects.update_or_create(
                        name=name,
                        defaults={
                            'title': manifest.get('name', name),
                            'description': manifest.get('description', ''),
...
def search_repos_by_topics(
    ...
    *,
    ...
    """Legacy wrapper for searching repositories."""
    config = GitHubConfig(token=token) if token else _DEFAULT_CLIENT.config
    client = GitHubClient(config) if token else _DEFAULT_CLIENT
    return client.search_repositories(topics, orgs, sort=sort, order=order, query=query)


def fetch_github_repositories(query: str) -> list[dict[str, Any]]:
    """Alias for searching repositories by query."""
    return _DEFAULT_CLIENT.search_repositories([], query=query)
...
def fetch_repo_manifests(
    ...
    """Legacy wrapper for fetching manifests."""
    config = GitHubConfig(token=token) if token else _DEFAULT_CLIENT.config
    client = GitHubClient(config) if token else _DEFAULT_CLIENT
    return client.fetch_manifests(repo)


def fetch_manifest_from_repo(full_name: str) -> dict[str, Any] | None:
    """Fetch the primary manifest from a repository."""
    manifests = _DEFAULT_CLIENT.fetch_manifests({"full_name": full_name})
    return manifests[0] if manifests else None


def sync_github_marketplace_items():
    """Legacy wrapper for syncing marketplace items."""
    return _DEFAULT_CLIENT.sync_marketplace_items()


def sync_marketplace_items():
    """Alias for syncing marketplace items."""
    return _DEFAULT_CLIENT.sync_marketplace_items()


def enrich_item_with_metrics(client: httpx.Client, owner: str, repo: str, item_dir: str, item: dict[str, Any]) -> None:
    """Legacy wrapper for enriching item with metrics."""
    _DEFAULT_CLIENT.enrich_item_with_metrics(client, owner, repo, item_dir, item)
