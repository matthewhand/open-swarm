...
class JeevesBlueprint(BlueprintBase):
    ...
    async def _run_non_interactive(self, messages: list[dict[str, Any]], **kwargs) -> Any:
        ...
        logger.info(
            "Running Jeeves non-interactively with instruction: %r",
            instruction[:100],
        )
        mcp_servers = kwargs.get("mcp_servers", [])
        # create_starting_agent() delegates model/profile resolution to
        # BlueprintBase.make_agent for every member of the Jeeves team.
