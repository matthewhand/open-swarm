...
=======
* **Team (`/v1/teams`)** — a dynamic **LLM-profile alias** (`id` / `description` / `llm_profile` in `teams.json`), also editable under Django `/teams/`. Appears as an OpenAI-compatible model id and proxies chat through `DynamicTeamBlueprint`. **Live `/teams/` remains this alias admin.**
>>>>>>> 3bca17e8 (fix(webui): open team composer from Grok chrome, not Home/Chat nav)
