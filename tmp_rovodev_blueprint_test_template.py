...
Usage: Copy this template and replace {BLUEPRINT_NAME} with actual blueprint name.
"""

import pytest
from unittest.mock import AsyncMock, MagicMock, patch
from src.swarm.blueprints.{BLUEPRINT_NAME}.blueprint_{BLUEPRINT_NAME} import {BLUEPRINT_CLASS}


class Test{BLUEPRINT_CLASS}BasicFunctionality:
    """Core functionality tests for {BLUEPRINT_NAME} blueprint."""
    
    def test_blueprint_initialization(self):
        """Test blueprint can be initialized with default parameters."""
        blueprint = {BLUEPRINT_CLASS}(blueprint_id="test_{BLUEPRINT_NAME}")
        assert blueprint is not None
        assert blueprint.blueprint_id == "test_{BLUEPRINT_NAME}"
    ...
    def test_blueprint_initialization_with_config(self):
        ...
        blueprint = {BLUEPRINT_CLASS}(blueprint_id="test_{BLUEPRINT_NAME}", config=config)
        assert blueprint is not None
...
class Test{BLUEPRINT_CLASS}AgentCreation:
    """Tests for agent creation and configuration."""
    
    @patch('src.swarm.blueprints.{BLUEPRINT_NAME}.blueprint_{BLUEPRINT_NAME}.{BLUEPRINT_CLASS}._get_model_instance')
...
class Test{BLUEPRINT_CLASS}Execution:
    """Tests for blueprint execution and run methods."""
    
    @pytest.mark.asyncio
    @patch('src.swarm.blueprints.{BLUEPRINT_NAME}.blueprint_{BLUEPRINT_NAME}.{BLUEPRINT_CLASS}.create_starting_agent')
...
class Test{BLUEPRINT_CLASS}Configuration:
...
class Test{BLUEPRINT_CLASS}Tools:
...
class Test{BLUEPRINT_CLASS}ErrorHandling:
    ...
    def test_initialization_with_invalid_config(self):
        ...
        try:
            blueprint = {BLUEPRINT_CLASS}(blueprint_id="test_{BLUEPRINT_NAME}", config=invalid_config)
...
class Test{BLUEPRINT_CLASS}Integration:
...
# - Test{BLUEPRINT_CLASS}CLI for command-line interface testing
# - Test{BLUEPRINT_CLASS}UX for user experience and spinner testing  
# - Test{BLUEPRINT_CLASS}Performance for performance-specific tests
# - Test{BLUEPRINT_CLASS}Delegation for multi-agent delegation flows