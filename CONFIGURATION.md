...
| `SWARM_CSP` | When `DEBUG=False`, enable `Content-Security-Policy` via `ContentSecurityPolicyMiddleware` (`script-src 'self'`; residual `style-src 'unsafe-inline'` for `style=""` attributes — `<style>` blocks are in `static/css/operator.css`; vendored Bootstrap/Prism/FA; no CDN). Set `false` to omit the header. | prod: on |
